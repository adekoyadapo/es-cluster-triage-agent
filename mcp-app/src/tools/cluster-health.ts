...
| EVAL used_pct = CASE(max_total_bytes > 0, 1.0 - min_available_bytes / max_total_bytes, NULL), pressure_score = COALESCE(indexing_pressure_bytes, 0) + COALESCE(merge_total, 0) * 100 + COALESCE(write_rejected, 0) * 1000 + COALESCE(search_rejected, 0) * 1000 + COALESCE(bulk_rejected, 0) * 1000 + COALESCE(segments_count, 0)
